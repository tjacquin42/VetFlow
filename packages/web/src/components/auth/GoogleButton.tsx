...
interface GoogleButtonProps extends Omit<ButtonHTMLAttributes<HTMLButtonElement>, 'onClick'> {
  onClick: () => Promise<void>;
  isLoading?: boolean;
  mode?: 'signin' | 'signup';
}

export function GoogleButton({ onClick, isLoading = false, mode = 'signin', ...props }: GoogleButtonProps) {
  const text = mode === 'signin' ? 'Se connecter avec Google' : "S'inscrire avec Google";
